...
msglogger.on('message', message => {
	if(message.author.id == msglogger.user.id) return;
	let n = new Date();let h = n.getUTCHours()+1;let m = n.getUTCMinutes();let s = n.getUTCSeconds();
	if (message.author.bot) return;if (message.channel.type == 'dm'){if(conf.logdmsmsg == true){console.log(`${chalk.red("[")}${chalk.red("DM ")}${chalk.blue(message.author.tag)}${chalk.red("]")}${chalk.red("[TIME ")}${chalk.blue(`${h}:${m}:${s}`)}${chalk.red("]")} : ${chalk.green(message.content)}`);};}else if(message.channel.type != 'dm'){if(conf.logguildsmsg == true){console.log(`${chalk.red("[")}${chalk.red("GUILD ")}${chalk.blue(message.guild.name)}${chalk.red("]")} ${chalk.red("[CHANNEL ")}${chalk.blue(message.channel.name)}${chalk.red("]")} ${chalk.red("[AUTHOR ")}${chalk.blue(message.author.tag)}${chalk.red("]")} ${chalk.red("[TIME ")}${chalk.blue(`${h}:${m}:${s}`)}${chalk.red("]")} : ${chalk.green(message.content)}`);};};
});
